...
No production window mode currently claims visually verified native Acrylic. The Phase 7 isolated `DesktopAcrylicController` proof of concept is stopped at Gate A because the unpackaged test process cannot resolve a Windows App Runtime Framework package in the current desktop-user package graph. Floating and Sidebar therefore retain the historical, unverified DWM request until a later phase can pass the isolated visual gate; Desktop continues to use translucent Graphite. Current-wallpaper mode is a separate rendered image background, not native transparency, and the wallpaper is reread on app/settings load rather than watched continuously. See [native-composition.md](native-composition.md) for the evidence and continuation criteria.
