...
Status: **Phase 1 implemented** — installation lifecycle, receipt, launch admission, manual bootstrap with v1.1 adoption, reinstall semantics, interrupted-install recovery, and native keep/remove-data uninstall implemented in commit `b381963` on top of v1.1.0 (`6c1de930ee82116198e592a38086adf3dcc62923`). **The Phase 2 signed updater's verification core is implemented in commit `df75c83`**: a shared pure-Rust protocol crate (`src-tauri/update-core`) linked by both the product binary and the maintenance helper, each of which verifies independently at runtime — strict closed `SignedEnvelopeV1`/`ManifestV1` parsing (the selected `windows-x64` asset is closed, the platform map key-space is open), canonical RFC 4648 Base64 signature validation, compiled read-only trust-store lookup with lowercase SHA-256 key ids, exact raw-manifest-byte Ed25519 verification before any parsing, semantic validation, a typed protocol error taxonomy, the candidate-ineligible/candidate-invalid distinction with newest-candidate fallthrough to older bridges, and the full eligibility predicate completing before target acceptance/freeze, all covered by deterministic golden and hostile fixtures; the shared canonical `Version` authority lives there too, so no second comparator exists. Its wire-level protocol remains frozen as Phase 2A final-freeze design in [Maintenance protocol v1](maintenance-protocol-v1.md) (canonical signed bytes, signature envelope, trust store, key rotation, session boundary, durable milestones, version selection, package rules). **Everything network- and transaction-side is not implemented** (§6–§10, §16): no production trust root is provisioned (the compiled store is an empty fail-closed placeholder), and no provider discovery, HTTP download, trusted-target persistence, `--expected-manifest-sha256` handoff, `UpdateSession` persistence, ZIP/package verification, exact package-allowlist enforcement, staging, helper replacement, `Updating` lifecycle integration, HealthAck, probation, rollback, frontend updater IPC, or release publishing code exists. Deviations between this document and the Phase 1 implementation are recorded inline as "Phase 1 notes"; the source-level audit in §2 describes the v1.1.0 baseline and is kept as the historical record.
...
Future logical components (placement to be decided during implementation): shared `maintenance-core` library with path policy, resource catalog, receipt codec, lock, journal/atomic writes, and integration reconciliation; separate transaction modules; native helper binary; updater module in the main backend. The signature-verification share of that core exists already: `desktop-todo-update-core` (implemented in commit `df75c83`) is the one pure protocol implementation both binaries link, each running its own verification at runtime — a "verified" claim from one process is coordination, never evidence for the other. Native UI is a helper adapter over core outcomes. Keep the four state machines independent rather than introducing a universal transaction engine.
...
**Phase 2B note (implemented in commit `df75c83`).** The shared verification core implements exactly the frozen schema-1 rules and nothing narrower. In particular, the package `filename` is a required signed field carried into the frozen target tuple; the frozen protocol states only that version, package name, and platform "must agree" and defines no concrete filename grammar, so no naming shape is a schema-1 manifest rejection. The filename stays an opaque identity/locator inside the signed manifest: the later package phase verifies the downloaded object against that already-signed metadata (filename equality, size, SHA-256) and the compiled package-content allowlist, and introduces no naming format that schema 1 never froze.
...
Two gates have tested implementations ahead of the updater: the persistence rollback-compatibility regression suite (settings unknown-key round-trip at top level, nested profiles, and individual Quick Links; rollback simulation of future-only keys; an additive future-schema fixture; schema-migration integrity under transient and persistent lock contention; settings update atomicity; controlled startup DB-failure semantics) is implemented and passing as of commit `8ff0301`, and the golden-raw-byte-fixture gate (exact-byte verification, hostile byte-mutation rejection, unknown key, K1-to-K2 bridge reachability, unsupported protocol, no stale-metadata downgrades) has a deterministic core-level implementation in the shared verification crate as of commit `df75c83`. The remaining gates are still open as written above — in particular the real-Windows E2E runs, probation integration, and the per-future-release compatibility review that decides hop eligibility; a passing regression suite does not certify any future release's migrations as rollback-safe, and a passing core fixture suite does not make any part of the network or update-transaction halves implemented.

Implementation slices are: (1) resource/path/receipt/lock primitives with Windows fault tests; (2) native install/uninstall/recovery and bootstrap; (3) signed protocol and provider adapters; (4) update/HealthAck/rollback with self-replacement QA; (5) release signing/mirror gate and Settings/About UI. Slices (1), (2), and the Phase 1 uninstall-entry portion of (5) are implemented. Slice (3)'s verification core — canonical signed bytes, signature envelope, trust store, strict closed parsing, version selection, and the candidate-eligibility predicate — is implemented in commit `df75c83` as the shared pure `desktop-todo-update-core` crate linked by both binaries; slice (3)'s provider adapters and bounded network enumeration, the session boundary and package rules within slice (3)'s wire protocol, slice (4) (update/HealthAck/rollback), and the release-signing/mirror portion of (5) remain unimplemented. Do not publish self-update until all mandatory safety gates pass.
