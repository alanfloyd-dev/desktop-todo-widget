...
//!
//! This is the Phase 1.5 canonical comparator, extracted verbatim from the
//! maintenance crate so the signed-update core can share it without creating
//! a second parser (protocol v1, "Installed version authority": "Protocol 1
//! introduces no semver crate and no second comparator"). The maintenance
//! crate re-exports this module; its receipts, bootstrap downgrade check, and
//! PE ProductVersion comparison all keep going through the same type.
...
impl Version {
    /// Construct from numeric components (test and PE-comparison use).
    ...
    /// The three numeric components, most significant first.
    pub fn components(&self) -> [u16; 3] {
...
mod tests {
    ...
    fn stable_versions_parse_and_order_semantically() {
        ...
        assert_eq!(parse("1.4.0").to_string(), "1.4.0");
        assert_eq!(parse("1.4.0").components(), [1, 4, 0]);
    }
    ...
    #[test]
    fn version_parse_errors_carry_the_version_kind() {
        assert_eq!(
            Version::parse("1.2").unwrap_err().kind,
            ErrorKind::VersionMalformed
        );
        assert_eq!(
            Version::parse("01.2.0").unwrap_err().kind,
            ErrorKind::VersionMalformed
        );
    }
