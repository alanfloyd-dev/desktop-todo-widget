...
#[cfg(target_os = "windows")]
type WindowsCompositionHostFactory = dyn Fn(&str, isize) -> windows_core::Result<Option<windows_core::IUnknown>>
  + Send
  + Sync
  + 'static;

#[cfg(target_os = "windows")]
type WindowsCompositionHostResize =
  dyn Fn(&str, isize, i32, i32) -> windows_core::Result<()> + Send + Sync + 'static;

#[cfg(target_os = "windows")]
static WINDOWS_COMPOSITION_HOST_FACTORY: std::sync::OnceLock<
  Box<WindowsCompositionHostFactory>,
> = std::sync::OnceLock::new();

#[cfg(target_os = "windows")]
static WINDOWS_COMPOSITION_HOST_RESIZE: std::sync::OnceLock<Box<WindowsCompositionHostResize>> =
  std::sync::OnceLock::new();

/// Translates one Win32 mouse message into a WebView2 `SendMouseInput` call.
///
/// Wry deliberately does not own this translation: coordinate decoding, wheel
/// delta extraction, capture and focus handling are application policy, and they
/// belong in the application's platform boundary. Wry only detects the message
/// and delegates here when composition hosting is active.
#[cfg(target_os = "windows")]
pub type WindowsCompositionMouseForwarder = dyn Fn(
...
#[cfg(target_os = "windows")]
static WINDOWS_COMPOSITION_MOUSE_FORWARDER: std::sync::OnceLock<
  Box<WindowsCompositionMouseForwarder>,
> = std::sync::OnceLock::new();

/// Registers the process-wide mouse-input forwarder used by composition hosting.
///
/// Must be registered before a composition-hosted WebView receives input.
#[cfg(target_os = "windows")]
pub fn set_windows_composition_mouse_forwarder<F>(forwarder: F) -> std::result::Result<(), &'static str>
where
  F: Fn(
      &webview2_com::Microsoft::Web::WebView2::Win32::ICoreWebView2Controller,
      &webview2_com::Microsoft::Web::WebView2::Win32::ICoreWebView2CompositionController,
    ...
    + 'static,
{
  WINDOWS_COMPOSITION_MOUSE_FORWARDER
    .set(Box::new(forwarder))
    .map_err(|_| "Windows composition mouse forwarder was already registered")
}

#[cfg(target_os = "windows")]
pub(crate) fn windows_composition_mouse_forwarder(
) -> Option<&'static WindowsCompositionMouseForwarder> {
  WINDOWS_COMPOSITION_MOUSE_FORWARDER
    .get()
    .map(|forwarder| forwarder.as_ref())
}

/// Registers a process-wide, Windows-only visual-host factory.
///
/// Wry remains windowed by default. Returning `Some` opts only that WebView
/// into CompositionController hosting. This must be registered before any
/// matching WebView is created.
#[cfg(target_os = "windows")]
pub fn set_windows_composition_host_factory<F>(factory: F) -> std::result::Result<(), &'static str>
where
  F: Fn(&str, isize) -> windows_core::Result<Option<windows_core::IUnknown>>
    + Send
    + Sync
    + 'static,
{
  WINDOWS_COMPOSITION_HOST_FACTORY
    .set(Box::new(factory))
    .map_err(|_| "Windows composition host factory was already registered")
}

/// Registers the matching geometry callback for an experimental Windows composition host.
#[cfg(target_os = "windows")]
pub fn set_windows_composition_host_resize_handler<F>(
  handler: F,
) -> std::result::Result<(), &'static str>
where
  F: Fn(&str, isize, i32, i32) -> windows_core::Result<()> + Send + Sync + 'static,
{
  WINDOWS_COMPOSITION_HOST_RESIZE
    .set(Box::new(handler))
    .map_err(|_| "Windows composition resize handler was already registered")
}

#[cfg(target_os = "windows")]
fn windows_composition_root_target(
  id: &str,
  parent: isize,
) -> windows_core::Result<Option<windows_core::IUnknown>> {
  match WINDOWS_COMPOSITION_HOST_FACTORY.get() {
    Some(factory) => factory(id, parent),
    None => Ok(None),
  }
}

#[cfg(target_os = "windows")]
fn windows_composition_host_resized(
  id: &str,
  parent: isize,
  width: i32,
  height: i32,
) -> windows_core::Result<()> {
  if let Some(handler) = WINDOWS_COMPOSITION_HOST_RESIZE.get() {
    handler(id, parent, width, height)?;
  }
  Ok(())
}
