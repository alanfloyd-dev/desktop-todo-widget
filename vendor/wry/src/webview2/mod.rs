...
impl Drop for InnerWebView {
  fn drop(&mut self) {
    if let Some(composition) = &self.composition_controller {
      let _ = unsafe { composition.SetRootVisualTarget(None::<&windows::core::IUnknown>) };
    }
...
impl InnerWebView {
  ...
  fn new_in_hwnd(
    ...
    let composition_root = crate::windows_composition_root_target(&id, parent.0 as isize)?;
    if composition_root.is_some() {
      unsafe {
        let _ = ShowWindow(hwnd, SW_HIDE);
      }
    }
    let controller_parent = if composition_root.is_some() { parent } else { hwnd };
    let (controller, composition_controller) = Self::create_controller(
      controller_parent,
      &env,
  ...
  fn create_controller(
    ...
    composition_root: Option<&windows::core::IUnknown>,
  ) -> Result<(
    ...
    if let Some(root) = composition_root {
      ...
      let handler = CreateCoreWebView2CompositionControllerCompletedHandler::create(Box::new(
        move |error_code, controller| {
          let result: windows::core::Result<ICoreWebView2CompositionController> = (|| {
            error_code?;
      ...
      let composition = webview2_com::wait_with_pump(rx)??;
      unsafe { composition.SetRootVisualTarget(root)? };
      let controller = composition.cast::<ICoreWebView2Controller>()?;
      eprintln!("[wry] webview_controller_type=ICoreWebView2CompositionController");
      return Ok((controller, Some(composition)));
    }
    ...
    let handler = CreateCoreWebView2ControllerCompletedHandler::create(Box::new(
      move |error_code, controller| {
        let result: windows::core::Result<ICoreWebView2Controller> = (|| {
          error_code?;
          controller.ok_or_else(|| windows::core::Error::from(E_POINTER))
        })();
        tx.send(result)
    ...
    eprintln!("[wry] webview_controller_type=ICoreWebView2Controller");
    Ok((controller, None))
  }
  ...
  fn init_webview(
    ...
    if !is_child {
      unsafe { Self::attach_parent_subclass(parent, &webview_id, controller) };
    }
    ...
    webview.add_WebResourceRequested(
      &WebResourceRequestedEventHandler::create(Box::new(move |_, args| {
          ...
          .find(|(protocol, _)| custom_protocol_workaround::is_work_around_uri(&uri, http_or_https, protocol))
        {
          ...
          let async_responder = Box::new(move |sent_response: HttpResponse<Cow<'static, [u8]>>| {
            let handler = move || {
    ...
    match msg {
      WM_SIZE | WM_DPICHANGED => {
        if msg != WM_SIZE || wparam.0 != SIZE_MINIMIZED as usize {
          let state = &*(dwrefdata as *mut ParentSubclassState);
          ...
          if state.composition.is_some() {
            if msg == WM_DPICHANGED {
              if let Ok(controller3) = state.controller.cast::<ICoreWebView2Controller3>() {
                let dpi = util::hwnd_dpi(hwnd);
                let _ = controller3.SetShouldDetectMonitorScaleChanges(false);
                let _ = controller3.SetRasterizationScale(util::dpi_to_scale_factor(dpi));
                eprintln!(
                  "[wry] composition_dpi_changed=true dpi={dpi} rasterization_scale={:.4}",
                  util::dpi_to_scale_factor(dpi)
                );
              }
            }
            let _ = crate::windows_composition_host_resized(
              &state.id,
              hwnd.0 as isize,
              width,
              height,
          ...
          // CompositionController hosting has no child controller HWND to keep
          // in sync: the WebView is a visual hosted by the application's
          // Compositor, and `SetBounds` above already sized it. Resizing the
          // controller's "parent window" here would resize the *top-level*
          // application HWND, which re-enters this same WM_SIZE handler and
          // collapses the window to zero. Windowed hosting (helper WRY_WEBVIEW
          // HWND) keeps the original behaviour.
          if state.composition.is_none() {
            let mut controller_parent = HWND::default();
            if state.controller.ParentWindow(&mut controller_parent).is_ok() {
              let _ = SetWindowPos(
                controller_parent,
                None,
                0,
      ...
      | WM_MOUSEWHEEL => {
        ...
        if let Some(composition) = &state.composition {
          // A composition-hosted WebView has no child HWND, so mouse input has
          // to be translated and forwarded. The translation lives in the
          // application's platform boundary
          // (`platform::windows::composition_host::input`) so Wry only detects
          // the message and delegates; the app registers it with
          // `set_windows_composition_mouse_forwarder` before creating WebViews.
          match crate::windows_composition_mouse_forwarder() {
            Some(forward) => {
              forward(
                &state.controller,
                composition,
      ...
      msg if msg == WM_DESTROY || msg == PARENT_DESTROY_MESSAGE => {
        ...
        if !(dwrefdata as *mut ()).is_null() {
          drop(Box::from_raw(dwrefdata as *mut ParentSubclassState));
  ...
  #[inline]
  unsafe fn attach_parent_subclass(parent: HWND, id: &str, controller: &ICoreWebView2Controller) {
    let state = ParentSubclassState {
      id: id.to_string(),
      controller: controller.clone(),
      composition: controller.cast::<ICoreWebView2CompositionController>().ok(),
    };
    let _ = SetWindowSubclass(
      parent,
      Some(Self::parent_subclass_proc),
      PARENT_SUBCLASS_ID as _,
      Box::into_raw(Box::new(state)) as _,
    );
  }
  ...
  pub fn reparent(&self, parent: isize) -> Result<()> {
    ...
    unsafe {
      ...
      if !self.is_child {
        ...
        Self::attach_parent_subclass(parent, &self.id, &self.controller);
